...
export default function FileDetailsPage({ fileId }: FileDetailsPageProps) {
  ...
  // Filter data based on priority and component (removed search)
  const filteredData = useMemo(() => {
    if (!file?.dataRecords) return [];
    ...
    return file.dataRecords.filter((record) => {
      ...
      return matchesPriority && matchesComponent;
    });
  }, [file?.dataRecords, priorityFilter, componentFilter]);
  ...
  // Clear all filters function (removed search clearing)
  const clearAllFilters = () => {
    setPriorityFilter("all");
    setComponentFilter("all");
  };
  ...
  if (!fileId) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 flex items-center justify-center p-4">
        <Card className="bg-white/80 backdrop-blur-lg border-0 shadow-2xl p-6 md:p-8 w-full max-w-md">
          <div className="flex items-center gap-3">
            <Loader className="w-6 h-6 animate-spin text-blue-600" />
            <span className="text-lg font-semibold text-slate-700">
              Loading...
  ...
  const CardView = () => (
    <div className="max-h-[400px] sm:max-h-[500px] lg:max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 pr-2">
            ...
            <CardHeader className="pb-3">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                <div className="flex-1 min-w-0">
                  <CardTitle className="text-base sm:text-lg font-semibold text-slate-800 mb-1 truncate">
                    {record.componentService}
                  </CardTitle>
  ...
  const TableView = () => (
    <div className="bg-white/80 backdrop-blur-lg rounded-lg shadow-lg overflow-hidden">
      <div className="max-h-[400px] sm:max-h-[500px] lg:max-h-[600px] overflow-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100">
        <table className="w-full">
          <thead className="bg-slate-50 sticky top-0 z-10">
            <tr>
              ...
              <th className="hidden sm:table-cell px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">
                Scenario
              </th>
              <th className="px-2 sm:px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">
                Priority
              </th>
              <th className="hidden md:table-cell px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">
                Description
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200">
            {filteredData.map((record, index) => (
              <tr
                ...
                className={index % 2 === 0 ? "bg-white" : "bg-slate-50/50"}
              >
                <td className="px-2 sm:px-4 py-3 text-sm font-medium text-slate-900">
                  <div className="truncate max-w-[120px] sm:max-w-none">
                    {record.componentService}
                  </div>
                  {/* Show scenario on mobile below component */}
                  <div className="sm:hidden text-xs text-slate-600 mt-1 truncate">
                    {record.scenarioName}
                  </div>
                </td>
                <td className="hidden sm:table-cell px-4 py-3 text-sm text-slate-700">
                ...
                <td className="hidden md:table-cell px-4 py-3 text-sm text-slate-700 max-w-xs">
                  <div className="line-clamp-2">{record.description}</div>
                </td>
              </tr>
            ))}
      ...
      <Card className="bg-white/80 backdrop-blur-lg border-0 shadow-lg">
        ...
        <CardContent>        
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {(priorityFilter !== "all" || componentFilter !== "all") && (
              <Button
                variant="outline"
                size="sm"
                onClick={clearAllFilters}
      ...
      {analytics && (
        <>
          ...
          <Card className="bg-white/80 backdrop-blur-lg border-0 shadow-lg">
            ...
            <CardContent>
              <div className="overflow-auto max-h-[400px] sm:max-h-[500px] scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100">
                <table className="w-full">
                  <thead className="bg-slate-50 sticky top-0 z-10">
                    <tr>
                      <th className="px-2 sm:px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">
                      ...
                      <th className="hidden lg:table-cell px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">
                        Possibility of Issues
                      </th>
                    </tr>
                  </thead>
                    ...
                    {filteredData.map((record, index) => (
                      ...
                      >
                        <td className="px-2 sm:px-4 py-3 text-sm font-medium text-slate-900">
                          ...
                          <div className="md:hidden text-xs text-slate-600 mt-1 line-clamp-2">
                            {record.description}
                          </div>
                        </td>
                        <td className="px-2 sm:px-4 py-3">
                        ...
                        <td className="hidden md:table-cell px-4 py-3 text-sm text-slate-700 max-w-md">
                          <div className="break-words">{record.description}</div>
                        </td>
                        <td className="hidden lg:table-cell px-4 py-3 text-sm text-slate-700 max-w-md">
                          <div className="break-words">
                            {record.possibilityOfIssues || "No issues identified"}
                          </div>
                        </td>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-6 py-4 sm:py-6 lg:py-8">
        {/* Header */}
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-4 gap-4 sm:gap-6">
          {/* File Information Card - Mobile stacked, Desktop sticky */}
          <div className="lg:sticky lg:top-6 lg:self-start order-1 lg:order-none">
            <Card className="bg-white/80 backdrop-blur-lg border-0 shadow-xl hover:shadow-2xl transition-all duration-300">
              ...
              <CardContent className="space-y-3 sm:space-y-4">
                <div className="flex items-center gap-3 p-3 bg-slate-50/50 rounded-lg">
                  <div className="w-6 h-6 sm:w-8 sm:h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-md flex items-center justify-center shrink-0">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-1 gap-2 sm:space-y-3 sm:gap-0">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-slate-600 text-xs sm:text-sm truncate">Records:</span>
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-slate-600 text-xs sm:text-sm truncate">Filtered:</span>
                  ...
                  <div className="col-span-2 sm:col-span-1 flex items-center gap-2 text-sm">
                    <Component className="w-4 h-4 text-slate-500 shrink-0" />
                    <span className="text-slate-600 text-xs sm:text-sm truncate">Components:</span>
                    <Badge variant="secondary" className="text-xs">{uniqueComponents.length}</Badge>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-3 sm:gap-4">
                    <div>
                      ...
                      <CardDescription className="text-sm">
                        Analyze component service data records
                      </CardDescription>
                    </div>
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
                        ...
                        <Select
                          ...
                          onValueChange={setComponentFilter}
                        >
                          <SelectTrigger className="w-full sm:w-48 border-2 border-slate-200 text-sm">
                            <Component className="w-4 h-4 mr-2 shrink-0" />
                            <SelectValue placeholder="All Components" />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="all">All Components</SelectItem>
                            {uniqueComponents.map((component) => (
                              <SelectItem key={component} value={component}>
                                <div className="truncate max-w-[200px]">{component}</div>
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                        ...
                        <Select
                          ...
                          onValueChange={setPriorityFilter}
                        >
                          <SelectTrigger className="w-full sm:w-36 border-2 border-slate-200 text-sm">
                      ...
                      <div className="flex flex-wrap gap-2 items-center">
                        ...
                        {(priorityFilter !== "all" || componentFilter !== "all") && (
                          <Button
                            ...
                            size="sm"
                            onClick={clearAllFilters}
                            className="ml-2 text-slate-600 border-slate-300 hover:bg-slate-50 text-xs sm:text-sm"
                          >
                            Clear All
                          </Button>
                        )}
                ...
                <CardContent>
                  {filteredData.length === 0 ? (
                    <div className="text-center py-8 sm:py-12">
                      <Database className="w-10 h-10 sm:w-12 sm:h-12 text-slate-300 mx-auto mb-4" />
